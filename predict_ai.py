    ...
    def predict_match(self, parent_match_id):   
        try:     
            ...
            if query:
                ...
                predicted_match = filtered_match if filtered_match["odd"] >= 1.15 and filtered_match["overall_prob"] >= 85 else None

                if int(predicted_match["sub_type_id"]) in [1, 45, 105] or int(predicted_match["outcome_id"]) in [10, 11, 13, 1711, 1713] or "&" in predicted_match["bet_pick"]:
