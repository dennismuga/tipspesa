    ...
    def predict_match(self, parent_match_id):   
        try:     
            ...
            if query:
                ...
                predicted_match = filtered_match if filtered_match["odd"] >=1.2 and filtered_match["overall_prob"]>=80 else None
                
                if int(predicted_match["sub_type_id"]) in [1, 18, 29] or int(predicted_match["outcome_id"]) in [10, 11, 13]:
